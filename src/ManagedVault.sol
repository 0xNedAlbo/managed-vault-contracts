...
contract ManagedVault is ERC4626Upgradeable {
    ...
    modifier onlyManager() {
        require(_msgSender() == _manager, "ManagedVault: Only allowed for manager");
        _;
    }
    ...
    function whitelistShareholder(address address_) public onlyManager {
        ...
        require(address_ != _manager, "ManagedVault: Manager cannot be shareholder");
    ...
    function loss(uint256 amount_) public onlyManager {
        require(amount_ <= _assetsInUse, "ManagedVault: Loss cannot be higher than assets in use");
        _assetsInUse -= amount_;
        emit Loss(amount_);
    }

    function fees(uint256 amount_) public onlyManager {
        require(amount_ <= _assetsInUse, "ManagedVault: Fees cannot be higher than assets in use");
        _assetsInUse -= amount_;
        emit Fees(amount_);
    }
    ...
    function setTotalAssets(uint256 amount_) public onlyManager {
        ...
        require(amount_ >= assetBalance, "ManagedVault: Assets in use cannot be less than vault balance");
